...
def style_maker(data, depth=0, replacer='  ', spaces_count=1):
        ...
        if isinstance(v, dict) and "action" in v:
            ...
            if method == 'record changed':
                lines.append(
                    ...
                    f"{style_maker(to_str(v.get('previous')), depth_size + 1)}"
                )
                lines.append(
                    f"{deep_indent}{prefixes['record added']}{k}: "
                    f"{style_maker(to_str(v.get('current')), depth_size + 1)}"
                )
            elif method == 'record nested':
                lines.append(
                    f"{deep_indent}{prefixes['record is the same']}{k}: "
                    f"{style_maker(to_str(v.get('children')), depth_size + 1)}"
                )
            else:
                lines.append(
                    f"{deep_indent}{prefixes[v.get('action')]}{k}: "
                    f"{style_maker(to_str(v.get('value')), depth_size + 1)}"
                )
        else:
            lines.append(
                f"{deep_indent}{prefixes['record is the same']}{k}: "
                f"{style_maker(to_str(v), depth_size + 1)}"
            )
